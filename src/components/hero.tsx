...
export function Hero({ isAnimated }: HeroProps) {
  ...
  return (
    ...
    >
      <div className="container mx-auto px-6 py-24 max-w-5xl flex flex-col md:flex-row items-center justify-between gap-8">
        <div className="md:w-1/2 space-y-6 animate-child">
          <h1 className="font-sans text-5xl md:text-5xl font-bold tracking-tight text-primary animate-child">
            Oladipupo Peace
          </h1>
          <p className="text-lg md:text-2xl font-bold text-muted-foreground leading-relaxed text-balance animate-child">
            Software Engineer
          </p>
          <p className="text-lg text-muted-foreground leading-relaxed text-balance animate-child">
            I build tech that works so naturally, you barely notice it.
          </p>
          <Link
            href="#about"
            className="inline-block px-6 py-3 bg-primary text-primary-foreground rounded-lg transition-all hover:bg-primary/80 font-sans text-lg tracking-tight"
          >
            About me
          </Link>
        </div>
